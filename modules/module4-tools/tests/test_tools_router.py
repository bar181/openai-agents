...
def test_add_endpoint():
    response = client.post("/tools/add", json={"a": 2, "b": 3}, headers=headers)
...
def test_multiply_endpoint():
    response = client.post("/tools/multiply", json={"a": 4, "b": 5}, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert "result" in data
...
def test_to_uppercase_endpoint():
    response = client.post("/tools/to_uppercase", json={"text": "hello"}, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert "result" in data
...
def test_concatenate_endpoint():
    response = client.post("/tools/concatenate", json={"text1": "hello", "text2": "world"}, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert "result" in data
...
def test_current_time_endpoint():
    response = client.get("/tools/current_time", headers=headers)
    assert response.status_code == 200
    data = response.json()
...
def test_add_days_endpoint():
    response = client.post("/tools/add_days", json={"base_date": "2023-01-01", "days": 5}, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert "result" in data
...
def test_echo_endpoint():
    response = client.post("/tools/echo", json={"message": "Hello World"}, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert "result" in data
...
def test_multi_tool_endpoint():
    ...
    response = client.post("/tools/multi-tool", json={"input_data": "2,3;hello;world"}, headers=headers)
    assert response.status_code == 200
    data = response.json()
